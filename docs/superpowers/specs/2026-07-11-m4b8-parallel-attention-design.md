...
| Kernel ABI | **Unchanged.** `inferno_attention_f32_{scalar,avx2}` are not modified; scalar↔AVX2 bit-identity and the `attn_rel_tol` derivation carry over untouched. `HOST_ABI_VERSION` bumps 4→5: numerics don't change, but the emitted code's host-call shape gains `inferno_par_attention`, and the constant's own rule is "bump whenever the host-call shape changes" (M4b.1/2/3 precedent) |
