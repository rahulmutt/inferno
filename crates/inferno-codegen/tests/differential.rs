...
/// kernel symbols the compiled `model.so` resolves against the host binary.
/// Without at least one reference the linker may drop `inferno-kernels`
/// entirely, leaving nothing to export and `dlopen` failing on the first
/// undefined `inferno_gemv_*` / `inferno_quantize_row_*` symbol.
